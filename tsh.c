...
{
	char *argv[MAXARGS];
	char buf[MAXLINE];	// local copy of cmdline
	int bg;
	pid_t pid;
	...
	if ( argv[0] == NULL )
		return;		// nothing to do with empty cmdline 	
...
int parseline(const char *cmdline, char **argv) 
{
	...
	*delim = '\0';
    ...
    argv[argc] = NULL;		// the last element of argv is NULL
...
int builtin_cmd(char **argv) 
{
	if ( !strcmp(argv[0], "quit" ) )
		exit(0);
	if ( !strcmp(argv[0], "&" ) )
		return 1;		// & means bg, but when it occurs alone, there is nothing to do
...
bg need to :
	1. parse the param
	2. rewrite the job state "BG"
	3. send the sig

fg need to :
	1. parse the param
	2. rewrite the job state to "FG"
	3. send the sig "SIGCONT"
	4. main process wait for the child 
	   ( call the waitfg function )
*/
	int jid = 0;
	pid_t pid = 0;
	struct job_t *job;
	...
	sigset_t mask,prev;
	Sigfillset(&mask);
	Sigprocmask(SIG_BLOCK, &mask, &prev);		// block all sigs	
	job->state = FG;
	kill(-pid, SIGCONT);
	waitfg(pid);		// unblock inside
	return;
}
...
void waitfg(pid_t pid)
{
	...
	fg_flag = 0;
	Sigprocmask(SIG_SETMASK, &mask, NULL);	//assume that it has been blocked, unblock now
...
pid_t Fork(void)
{
	...
	if ( ( pid = fork() ) < 0)
	{
		unix_error("Fork error");	//when error occurs, it will not return
	}
	return pid;
}
...
{
	int res;
	if ( (res = sigprocmask(how, set, oldset)) < 0 )
...
{
	int res;
	if ( (res = sigismember(set, signum)) < 0)
